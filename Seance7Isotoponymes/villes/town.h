...
// Ajout de l'opérateur < pour permettre l'utilisation dans std::set
bool operator<(const Town& a, const Town& b);

std::set<Town> calculateN(const std::vector<Town>& towns);
std::set<Town> calculateC(const std::vector<Town>& towns);
std::set<Town> intersectionNC(const std::set<Town>& N, const std::set<Town>& C);
